...
export function CartoesDeVisitaEstandesProfessional({ estande, jwt, getEstande, idUserMatchesIdArray }: EstandeProps) {
  ...
  const handleCreateCartao: SubmitHandler<CreateCartaoFormData> = async (data) => {
    const values = {
      ...data, 
      estandes_professional: estande.id
    };
  ...
  const deleteUser = async (id: string) => {
    ...
    axios.delete(`${process.env.NEXT_PUBLIC_API}/cartoes-estande-professionals/${id}`, authorization)
      .then(response => {
      estande.cartoes_estande_professionals.filter(cartao => id !== cartao.id)
  ...
  return (
    ...
    {estande.cartoes_estande_professionals?.map(cartao => {
      return (
        <>
        {!showCreateCartao && (
          <VStack
