...
import { CartoesDeVisitaEstandesPremium } from "./CartoesDeVisitaEstandePremium";
import { useAuth } from "../../../hooks/useAuth";

type ImagesEstandesPremiumProps = {
  ...
  imagePedestal: string
  idUserMatchesIdArray: boolean
}

export function ImagesEstandesPremium({ 
  jwt, 
  estande, 
  getEstande, 
  imageVideo, 
  ...
  const [editingPedestal, setEditingPedestal] = useState(false)
  ...
  const handleImageUpdateSubmit: SubmitHandler<any> = async (data) => {
		const formData = new FormData()
			...
			formData.append('ref', 'estandes-premium');
  ...
  return (
    <>
      ...
      <Box p='8' pb='4' bg={bgColor[colorMode]} borderRadius={8}>
        <Text fontSize='lg' mb='4'>Image pedestal</Text>
        <AspectRatio maxW="555px" ratio={4 / 3}>
          <Image src={`${imagePedestal}`} alt="imagem_pedestal" objectFit="cover" />
        </AspectRatio>
        <form key={5} onSubmit={handleSubmit5(handleImageUpdateSubmit)}>
        <HStack justify='space-between' mt='3'>
          <Input 
            variant='flushed' 
            ref={register5({ required: true })} 
            type="file" 
            size='sm'
            disabled={!idUserMatchesIdArray && userRole !== 'Administrator'}
            name='files' 
            onClick={() => setEditingPedestal(true)}
          />
          {editingPedestal && (
            <HStack spacing='2'>
              <IconButton 
                aria-label="close editing" 
                onClick={() => {setEditingPedestal(false); reset5()}}
                size='sm'
                icon={<RiCloseLine />} 
              />
              <IconButton 
                ...
                onClick={() => {setStrapiField('imagem_pedestal')}}
                icon={<RiCheckLine />} 
              />
            </HStack>
          )}
            ...
            {editingLogoDaEmpresa && (
              <HStack spacing='3'>
                ...
                <IconButton 
                  ...
                  onClick={() => {setStrapiField('logo_empresa')}}
                  icon={<RiCheckLine />} 
                />
              </HStack>
            )}
      ...
      <Box pb='8'>
        ...
        <CartoesDeVisitaEstandesPremium
          estande={estande} 
          jwt={jwt} 
